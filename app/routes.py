...
def login():
    ...
    if form.validate_on_submit():
        ...
        cursor = conn.cursor()
        Checkemail = email=form.email.data
        Checkpassword = password=form.password.data
        print(Checkemail)
        print(Checkpassword)
        cursor.execute(f"SELECT * FROM customer WHERE email='{Checkemail}' AND password='{Checkpassword}'")
        result = cursor.fetchall()
        if result != []:
                #if authenticated
                getdata = result[0]
                user = Customer.query.filter_by(email=getdata[3]).first()
                login_user(user, remember=True)
                next_page = request.args.get("next")
                return redirect(next_page) if next_page else redirect(url_for('customerAccount'))
        cursor.execute(f"SELECT * FROM employee WHERE email='{Checkemail}' AND password='{Checkpassword}'")
        result = cursor.fetchall()
        if result != []:
            #if authenticated
            getdata = result[0]
            user = Employee.query.filter_by(email=getdata[3]).first()
            login_user(user, remember=True)
        ...
        #     # finding the associated account if it is in the database anot
        #     Checkemail = email=form.email.data
        #     Checkpassword = password=form.password.data
        #     print(Checkemail)
        #     print(Checkpassword)
        ...
        #     print(Checkemail)
        #     print(Checkpassword)
        #     cursor.execute("SELECT * FROM employee WHERE email='{Checkemail}' AND password='{Checkpassword}'")
        #     result = cursor.fetchall()
        #     if result != []:
        #         #if authenticated
